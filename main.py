...
👥 克隆体：{len(player['clones'])}/{player['max_clones']}个"""
    ...
    SHIP_BLUEPRINTS = {
        "狂风级护卫舰": {
            ...
            "materials": {
                # 安全区矿物（100%）
                "硫": 300.0, "铁": 120.0, "镁": 60.0, "硅": 48.0,
                "铝": 36.0, "锰": 24.0, "锌": 12.0
            },
            "skill": "护卫舰操控理论",
            "time": 0.27  # 16秒 ≈ 0.27分钟
        },
        "蜜蜂级采矿护卫舰": {
            "id": 5,  # 保持ID不变
            "type": "采矿",
        ...
        "骤雨级驱逐舰": {
            ...
            "cost": 3000000,  # ¥300万
            "materials": {
                "硫": 600.0, "铁": 240.0, "镁": 120.0, "硅": 96.0,
                "铝": 72.0, "锰": 48.0, "锌": 24.0
            },
            "skill": "驱逐舰操控理论",
            "time": 0.53  # 32秒 ≈ 0.53分钟
        },
        "崆峒级运输舰": {
            "id": 7,
            "type": "运输",
            "cost": 6000000,  # ¥600万
            "materials": {
                "硫": 1200.0, "铁": 480.0, "镁": 240.0, "硅": 192.0,
                "铝": 144.0, "锰": 96.0, "锌": 48.0
            },
            "skill": "运输舰操控理论",
            "time": 1.07  # 64秒 ≈ 1.07分钟
        },
        "烈火级巡洋舰": {
            "id": 3,
            "type": "作战",
            ...
            "materials": {
                # 安全区矿物（75%）
                "硫": 3000.0, "铁": 1200.0, "镁": 600.0, "硅": 480.0,
                "铝": 360.0, "锰": 240.0, "锌": 120.0,
                # 非安全区矿物（25%）
                "铅": 138.9, "钛": 34.7, "铬": 27.8, "锑": 23.1,
                "锂": 5.0, "铜": 8.9, "镍": 3.8, "锡": 2.3,
                "钴": 1.0, "钼": 0.3, "铌": 0.1, "钨": 0.1, "钒": 0.002
            },
            "skill": "巡洋舰操控理论",
            "time": 3.55  # 3分33秒 ≈ 3.55分钟
        },
        "蝗虫级采矿驳船": {
            "id": 6,
            "type": "采矿",
            ...
            "materials": {
                # 安全区矿物（75%）
                "硫": 12000.0, "铁": 4800.0, "镁": 2400.0, "硅": 1920.0,
                "铝": 1440.0, "锰": 960.0, "锌": 480.0,
                # 非安全区矿物（25%）
                "铅": 555.6, "钛": 138.9, "铬": 111.1, "锑": 92.6,
                "锂": 20.0, "铜": 35.7, "镍": 15.4, "锡": 9.3,
                "钴": 4.0, "钼": 1.3, "铌": 0.5, "钨": 0.3, "钒": 0.008
            },
            "skill": "采矿驳船操控理论",
            "time": 14.22  # 14分13秒 ≈ 14.22分钟
        },
        "怒雷级战列舰": {
            "id": 4,
            "type": "作战",
            ...
            "materials": {
                # 安全区矿物（67%）
                "硫": 53600.0, "铁": 21440.0, "镁": 10720.0, "硅": 8576.0,
                "铝": 6432.0, "锰": 4288.0, "锌": 2144.0,
                # 非安全区矿物（33%）
                "铅": 3666.7, "钛": 916.7, "铬": 733.3, "锑": 611.1,
                "锂": 132.0, "铜": 235.7, "镍": 101.5, "锡": 61.1,
                "钴": 26.4, "钼": 8.8, "铌": 3.0, "钨": 1.5, "钒": 0.05
            },
            "skill": "战列舰操控理论",
            "time": 71.11  # 1小时11分 ≈ 71.11分钟
        },
        "泰山级货舰": {
            "id": 8,
            "type": "运输",
            ...
            "materials": {
                # 安全区矿物（67%）
                "硫": 603000.0, "铁": 241200.0, "镁": 120600.0, "硅": 96480.0,
                "铝": 72360.0, "锰": 48240.0, "锌": 24120.0,
                # 非安全区矿物（33%）
                "铅": 41250.0, "钛": 10312.5, "铬": 8250.0, "锑": 6875.0,
                "锂": 1485.0, "铜": 2651.8, "镍": 1142.3, "锡": 687.5,
                "钴": 297.0, "钼": 99.0, "铌": 33.7, "钨": 16.9, "钒": 0.56
            },
            "skill": "货舰操控理论",
            "time": 800.0  # 13小时20分 = 800分钟
        }
    }
    ...
    async def list_ships(self, event: AstrMessageEvent):
        ...
        if len(args) >= 1:
            ...
            # 格式化时间显示
            time_minutes = blueprint['time']
            if time_minutes < 1:
                time_str = f"{int(time_minutes * 60)}秒"
            ...
            # 分离安全区和非安全区矿物
            safe_minerals = ["硫", "铁", "镁", "硅", "铝", "锰", "锌"]
            safe_materials = []
            unsafe_materials = []
            
            for mineral, tons in blueprint['materials'].items():
                if mineral in safe_minerals:
                    safe_materials.append(f"  {mineral}：{tons:.1f}吨")
                else:
                    unsafe_materials.append(f"  {mineral}：{tons:.1f}吨")
            
            materials_display = "【安全区矿物】\n" + "\n".join(safe_materials)
            if unsafe_materials:
                materials_display += "\n\n【非安全区矿物】\n" + "\n".join(unsafe_materials)
